...
本项目是 **LLM 推理方案的性能决策基线**：不只实现 Attention 论文组件，还将首 token 延迟、单 token 延迟、吞吐、显存和质量一致性拆开测，支持 Cache、Attention 和解码策略的工程取舍。
...
**2026-08-20 文档更新：** 复现入口统一使用仓库相对路径，新增模型质量证据探针说明；TTFT/TPOT、
Cache、CUDA 和质量结果仍按硬件与运行模式分栏，不合并成未经验证的线上收益。
